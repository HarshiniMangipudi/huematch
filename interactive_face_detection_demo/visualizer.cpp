...
void Visualizer::drawFace(cv::Mat& img, Face::Ptr f, bool drawEmotionBar) {
    ...
    if (f->isLandmarksEnabled()) {
        ...
        int x6 = 0, y6 = 0, x20 = 0, y20 = 0, xmid = 0, ymid = 0; 
        ...
        //get the midpoints of the necessary landmarks 
        xmid = ( (x6 + x20) / 2 ); 
        ymid = ( (y6 + y20) / 2 );
        //rectangle boundary of pixels we want to find avg color of 
        cv::rectangle( img, cv::Point(xmid-DIFF, ymid-DIFF ), cv::Point( xmid+DIFF, ymid), cv::Scalar( 0, 0 , 0 ),0);
        
        //crop the image to the boundary of pixels we want to find the avg color of 
        cv::Mat image(img); 
        cv::Rect roi( cv::Point(xmid-DIFF, ymid-DIFF), cv::Point( xmid+DIFF, ymid)); 
        cv::Mat cropImg = image(roi); 
        //create a mask over the cropped boundary 
        cv::Mat1b mask(cropImg.rows, cropImg.cols);
        //find the average hue of the mask and display 
        //a rectangle with that color 
        cv::Scalar mean = cv::mean(cropImg, mask);
        cv::rectangle( img, cv::Point(100, 100 ), cv::Point( 200, 150), mean,-1);
        
        //read in the file containing make up brands foundations 
        std::ifstream makeupBrands; 
        //enter path to file here 
        makeupBrands.open("/home/vpro2/Documents/makeupBrands.txt");
        std::string line = ""; 
        
        //read in the rgb values of each foundation 
        //line by line 
        std::vector<int> rgbVals; 
        bool found = false; 
        while(std::getline(makeupBrands, line) && !found ){
            //store the rgb values in an array and convert them
            //to bgr to fit the opencv format
            std::stringstream rgbLine( line );
            std::copy(  std::istream_iterator<int>(rgbLine), 
                        std::istream_iterator<int>(),
                        std::back_inserter(rgbVals));
            std::reverse(rgbVals.begin(),rgbVals.end());

            //check if the rgb values of the foundation are within
            //the range of the rgb values of the average hue of face
            //if so there is a succesful match with foundation!
            bool checkMatch = true; 
            for(unsigned int i = 0; i < rgbVals.size(); i++){
                if( !( (rgbVals[i] >= mean[i] - RANGE) && (rgbVals[i] <= mean[i] + RANGE) ) ){
                       checkMatch = false; 
                }
            }
            if(checkMatch){
                found = true;
                //enter path to pictures here
                std::string fdPath = "/home/vpro2/Documents/foundationPics/" + line + ".jpg";  
                //if there was a match display the matched foundation 
                cv::Mat foundation = cv::imread(fdPath, cv::IMREAD_COLOR);
                cv::Mat foundationResized; 
                cv::resize(foundation, foundationResized, cv::Size(img.size().width, img.size().height)); 
                cv::imshow("Display foundation", foundationResized);
                std::cout << "Your hue is #" << std::hex << rgbVals[2] << rgbVals[1] << rgbVals[0] << std::endl; 
            }
            else{
                std::cout << "Your hue is #" << std::hex << mean[2] << mean[1] << mean[0] << std::endl; 
                rgbVals.clear(); 
            }
